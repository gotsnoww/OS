...
int main(int argc, char *argv[])
{
  ...
  //hw3가 4개의 인자를 받지 못할시 에러메시지 출력 후 종료
  if (argc != 5) {
	fprintf(stderr, "Usage: ./hw3 infile1 infile2 outfile <p>");
	exit(1);	
  }

  int t = atoi(argv[4]); //thread 인자의 값이 0 이하일 시 에러 메시지 출력
  if (t <= 0) {
  ...
  if(t > (int)n){ //작업 thread의 개수가 행렬의 한쪽 면의 크기인 n보다 클 경우 thread의 개수를 n으로 설정
	  t = (int)n;
  }
